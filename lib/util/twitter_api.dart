...
class twitterApi {
  ...
  final _oauth_signature_method =
      "HMAC-SHA1"; //ignore: non_constant_identifier_names
  ...
  getTwitterRequest(String method, String url,
    ...
    if (method.toUpperCase() == "GET") {
      ...
      // hang forever
      response = get(Uri.https("api.twitter.com", "/1.1/" + url, options),
          headers: {
  ...
  _percentEncode(String val) {
    ...
    for (var i = 0; i < bytes.length; i++) {
      ...
      if (!_percentEncodeValues.contains(char)) {
        ...
        for (var i = 0; i < char.toRadixString(16).length; i++) {
          output += char.toRadixString(16)[i].toUpperCase();
        }
      } else {
